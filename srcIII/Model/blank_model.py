...
class Model_Blank(nn.Module):
    def __init__(self, model_config, model_parameters, unembedding_data):
        ...
        # vocab_size = model_parameters['embeddings']['token_embed']['embeddings'].shape[0] - 1
...
class DINOv2_Image_Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        patch_features = features["x_norm_patchtokens"][:, self.indices, :]  
        
        logits = self.projection(patch_features).squeeze(-1)  
        probabilities = torch.sigmoid(logits)
        hard_decisions = (probabilities > 0.5).long()
        ste_decisions = hard_decisions - probabilities.detach() + probabilities  
        
        prefix = torch.tensor([[3]], device=x.device).expand(batch_size, 3)
        tokens = torch.cat([prefix, ste_decisions], dim=1)  
