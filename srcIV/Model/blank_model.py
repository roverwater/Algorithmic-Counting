...
class Model_Blank(nn.Module):
    def __init__(self, model_config, model_parameters, unembedding_data):
        super(Model_Blank, self).__init__()
        self.image_transformer = DINOv2_Image_Transformer().to(config.device)
        self.embedding_dim = unembedding_data.shape[0]
        self.embedder = Embedding_Module(
            pos_embed_data=model_parameters['embeddings']['pos_embed']['embeddings'], 
        ...
        self.unembedder = Unembedding_Module(
            unembedding_data=unembedding_data,
            use_unembed_argmax=False,
            trainable=False
        )
        # Ensure embedder is part of the model to access its embeddings
        self.image_transformer.embedder = self.embedder

    def forward(self, x):
        x = self.image_transformer(x)  # Directly outputs embeddings
        x = self.transformer(x)
        x = self.unembedder(x)
        return x
    
class DINOv2_Image_Transformer(nn.Module):
    def __init__(self, target_token_id=1):
        super().__init__()
        self.model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14').to(config.device)
        self.model.eval()
        ...
        self.grid_size = 16
        self.n_patches = 6  
        start = (self.grid_size - self.n_patches) // 2
        end = start + self.n_patches
        self.indices = [row * self.grid_size + col for row in range(start, end) for col in range(start, end)]
        
        # Projection to two logits (for 0 and 1)
        self.projection = nn.Sequential(
            nn.Linear(384, 128),
            nn.GELU(),
            nn.Linear(128, 4)  # Output two logits per patch
        )
    ...
    def forward(self, x):
        ...
        patch_features = features["x_norm_patchtokens"][:, self.indices, :]  # [B, n_patches, 384]
        
        logits = self.projection(patch_features)  # [B, n_patches, 2]

        # Gumbel-Softmax with straight-through estimator
        hard_samples = F.gumbel_softmax(logits, tau=0.1, hard=True)  # [B, n_patches, 2]

        # Get token embeddings from the embedder
        embed_matrix = self.embedder.token_embed.weight  # [vocab_size, embed_dim]
        token_embeds = torch.matmul(hard_samples, embed_matrix)  # [B, n_patches, embed_dim]

        # Prepend BOS embeddings
        bos_token = torch.tensor([config.encoded_bos], device=x.device)
        bos_embed = self.embedder.token_embed(bos_token)  # [1, embed_dim]
        bos_embeds = bos_embed.unsqueeze(0).expand(batch_size, 1, -1)  # [B, 1, embed_dim]

        combined_embeds = torch.cat([bos_embeds, token_embeds], dim=1)  # [B, 1 + n_patches, embed_dim]

        # Add positional embeddings
        positions = torch.arange(combined_embeds.shape[1], device=x.device)
        pos_embeds = self.embedder.pos_embed(positions)  # [seq_len, embed_dim]
        pos_embeds = pos_embeds.unsqueeze(0).expand(batch_size, -1, -1)  # [B, seq_len, embed_dim]

        x = combined_embeds + pos_embeds  # [B, seq_len, embed_dim]
        return x
...
class Transformer_Module(nn.Module):
    def __init__(self, model_config, model_parameters, trainable=False):
