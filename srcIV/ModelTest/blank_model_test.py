...
from srcIV.Utils import RASPModelSimple, utils 
...
# Move model to GPU
model = blank_model.Model_Blank(model_config=config.model_config,
...
def convert_label_to_logits(batch_labels):
    ...
    one_hots = F.one_hot(batch_labels, num_classes=config.output_dim).float()
    labels[:, 1:] = one_hots.unsqueeze(1).expand(-1, final_output_size - 1, -1)
    return labels

def train_model():
    model.train()  
    gradient_magnitudes = {}  # Track gradients per layer
    for epoch in range(config.num_epochs):
        ...
        for batch in train_loader:
            ...
            if config.logit_dataset:
                ...
                # ax1.imshow(inputs[0].permute(1, 2, 0).cpu().numpy())                
                # ax1.set_title(f"{labels}", fontsize=12)
                # ax1.axis('off')
                ...
                with torch.no_grad():
                    for name, param in model.named_parameters():
                        if param.grad is not None:
                            ...
                            if name not in gradient_magnitudes:
                                gradient_magnitudes[name] = []
                            gradient_magnitudes[name].append(grad_norm)
                
                    optimizer.step()
                    
                    # Print gradients every 10 batches
                    if batch_idx % 10 == 0:
                        print(f"\nBatch {batch_idx} Gradient Summary:")
                        for name, norms in gradient_magnitudes.items():
                            if "weight" in name and "projection" in name:
                                print(f"{name:50} {np.mean(norms[-10:]):.2e}")
                        gradient_magnitudes.clear()  # Reset for next batch group
                ...
                flattened_labels = selected_labels.contiguous().view(-1, selected_labels.size(-1)).to(config.device)

                log_probs = torch.nn.functional.log_softmax(flattened_outputs, dim=1).to(config.device)
        ...
        plot_gradient_distribution(epoch)
        evaluate_model()
def plot_gradient_distribution(epoch):
    ...
    for name, param in model.named_parameters():
        if param.grad is not None:
            grads.append(param.grad.norm().log10().item())
            names.append(name)
    
    # Create plot
    plt.barh(names, grads)
    plt.title(f"Gradient Magnitudes (Log Scale) - Epoch {epoch+1}")
