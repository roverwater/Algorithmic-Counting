...
#             seq_len = np.random.randint(4, config.max_seq_len + 1)  
#             # sequence = [config.bos, config.tmp, config.sep] + [str(x) for x in np.random.choice(list(config.vocab_tokens), seq_len - 3, replace=True)]
#             sequence = (
#                 [config.bos, config.tmp, config.sep] +
#                 [
#                     config.token_to_count if np.random.rand() < 0.7 else str(np.random.choice(list(config.vocab_tokens), replace=True))
...
#             ground_truth = torch.zeros(config.max_seq_len, config.output_dim)
#             if count_a < config.output_dim:
...
#     return list(inputs), stacked_labels  
...
#     config.train_loader = DataLoader(train_dataset, batch_size=config.batch_size, shuffle=True, collate_fn=collate_fn)
#     config.test_loader = DataLoader(test_dataset, batch_size=config.batch_size, shuffle=False, collate_fn=collate_fn)

#     print("Train Data:")
#     for batch in config.train_loader:
#         inputs, labels = batch
#         # print("Inputs:", inputs)
#         # print("Labels (argmax):", labels.argmax(2))
...
# Include generate_image function and polygon import here
...
class ImageCountingDataset(Dataset):
    def __init__(self, num_samples_per_label=10, labels=range(7)):
        ...
        for label in labels:
            ...
            for _ in range(num_samples_per_label):
                img, actual_label = generate_image(label, num_distractors=0, num_extra_triangles=0, num_diamonds=0)
                processed = self._process_image(img)

                int_list = processed.detach().int().numpy().tolist()
                str_list = [str(x) for x in int_list]

                sequence = (
                [config.bos, config.tmp, config.sep] + str_list
                )
                ...
                if (str_list.count(config.token_to_count) + 1 == count_a):
                    self.data.append(sequence)
                    self.labels.append(ground_truth)
            ...
            config.untrue = untrue            

    def _process_image(self, img):
        """Convert 32x32 image to 1x25 tensor"""
        tensor = torch.from_numpy(img).unsqueeze(0).unsqueeze(0).float()  # (1,1,32,32)
        ...
        tensor = self.pool(tensor)       # (1,1,5,5) after maxpool
        return tensor.flatten()          # (25,) -> Treated as 1D sequence
...
def GenerateDataset():
    ammount_red = 8
    num_samples_per_label = config.batch_size // ammount_red
    full_dataset = ImageCountingDataset(
        num_samples_per_label=num_samples_per_label,
        labels=range(ammount_red)
    )
...
def train_model():
    ...
    for epoch in range(config.num_epochs):
        ...
        print(f"Epoch {epoch + 1}/{config.num_epochs}, Untrue: {config.untrue}, Temp: {current_temp:.3f}, Loss: {total_loss / len(config.train_loader):.4f}")
        evaluate_model()
