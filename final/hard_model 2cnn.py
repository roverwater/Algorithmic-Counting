...
from trainable import Model_1D_trainable
...
loss_cnn_model = []
loss_cnn_reg_model = []
loss_transformer_cls_model = []
loss_transformer_reg_model = []
loss_trainable_rasp = []
...
def evaluate_model():
    ...
    acc_rasp_model.append(correct / total)
...
class CNNRegressor(nn.Module):  # New regression model
    def __init__(self, vocab_size, max_count, embed_dim=64, num_filters=128, kernel_size=3):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim)
        self.conv1 = nn.Conv1d(embed_dim, num_filters, kernel_size, padding=kernel_size//2)
        self.conv2 = nn.Conv1d(num_filters, num_filters, kernel_size, padding=kernel_size//2)
        self.regressor = nn.Linear(num_filters, 1)
...
class SimplePositionalEncoding(nn.Module):
    def __init__(self, embed_dim, max_seq_len=38):
        super().__init__()
...
class SimpleTransformerClassifier(nn.Module):
    def __init__(self, vocab_size, max_count, embed_dim=64, num_heads=4):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim)
        self.pos_encoder = SimplePositionalEncoding(embed_dim)
        ...
        self.classifier = nn.Linear(embed_dim, max_count + 1)
    ...
    def forward(self, x, temperature=1.0):
        ...
        pooled = output.mean(dim=0)  # Average pooling
        return self.classifier(pooled) / temperature
...
if __name__ == "__main__":
    ...
    model_cnn_reg = CNNRegressor(vocab_size=7, max_count=25).to(config.device)

    model_transformer_cls = SimpleTransformerClassifier(
        vocab_size=7, max_count=25, embed_dim=64, num_heads=4
    ).to(config.device)
    
    model_transformer_reg = SimpleTransformerRegressor(
        vocab_size=7, max_count=25, embed_dim=64, num_heads=4
    ).to(config.device)

    # Shared label converter
    def convert_labels(inputs, for_regression=False):
        counts = [seq[3:].count(config.token_to_count) for seq in inputs]
        dtype = torch.float32 if for_regression else torch.long
        return torch.tensor(counts, dtype=dtype).to(config.device)
    
    # Training setup for both CNN models
    def train_cnn_model(model_cnn, is_regression=False, loss_list=None, acc_list=None):
        model_cnn.train()
        ...
        for epoch in range(config.num_epochs):
            ...
            for batch in config.train_loader:
                inputs, _ = batch
                labels = convert_labels(inputs, for_regression=is_regression)
                input_indices = model.encoder(inputs)
            ...
            print(f"{'Regression' if is_regression else 'Classification'} Epoch {epoch+1} Loss: {total_loss/len(config.train_loader):.4f}")
            # Store losses separately
            if loss_list is not None:
                loss_list.append(total_loss/len(config.train_loader))
    ...
    def evaluate_all_models():
        models = {
            'CNN Classifier': model_cnn,
            'CNN Regressor': model_cnn_reg,
            'Transformer Classifier': model_transformer_cls,
            'Transformer Regressor': model_transformer_reg
        }
        
        results = {}
        for name, model_test in models.items():
            if 'Regressor' in name:
                acc = evaluate_regression_model(model_test)
            else:
                acc = evaluate_classification_model(model_test)
            results[name] = acc
        
        print("Validation Accuracies:")
        for name, acc in results.items():
            print(f"- {name}: {acc:.4f}")
            if 'Regressor' and 'CNN' in name:
                acc_cnn_reg_model.append(acc)
            elif 'Regressor' and 'Transformer' in name:
                acc_transformer_reg_model.append(acc)
            elif 'Classifier' and 'CNN' in name:
                acc_cnn_model.append(acc)
            elif 'Classifier' and 'Transformer' in name:
                acc_transformer_cls_model.append(acc)
            else:
                raise Exception("Failed")
        print()

    # Specialized evaluation functions
    def evaluate_classification_model(model_tested):
        model_tested.eval()
        correct = 0
        total = 0
        with torch.no_grad():
            for batch in config.test_loader:
                inputs, _ = batch
                true_counts = convert_labels(inputs)
                inputs = model.encoder(inputs)
                outputs = model_tested(inputs)
                predicted = outputs.argmax(-1)
                correct += (predicted == true_counts).sum().item()
                total += true_counts.numel()
        return correct / total

    def evaluate_regression_model(model_tested):
        model_tested.eval()
        correct = 0
        total = 0
        with torch.no_grad():
    ...
    def train_model_train(model_trainable):
        model_trainable.train()  
        optimizer = optim.Adam(model_trainable.parameters(), lr=1e-3)
        for epoch in range(config.num_epochs):
            current_temp = get_current_temperature(
                epoch, 
                config.num_epochs, 
            ...
            for batch in config.train_loader:
                inputs, labels = batch
                inputs, labels = inputs, labels.to(config.device)
                outputs = model_trainable(inputs, temperature=current_temp)
                mask = torch.zeros_like(outputs)
                mask[:, 1:, :] = 1
                outputs_modified = outputs * mask + (outputs * (1 - mask)).detach()
                outputs = outputs_modified
                ...
                log_token_preferences(model_trainable, epoch, token_history)
            print(f"Epoch {epoch + 1}/{config.num_epochs}, Temp: {current_temp:.3f}, Loss: {total_loss / len(config.train_loader):.4f}")
            loss_trainable_rasp.append(total_loss / len(config.train_loader))
            evaluate_model_train(model_trainable)

    def evaluate_model_train(model_trainable):
        ...
        with torch.no_grad():
            for batch in config.test_loader:
                ...
                outputs = model_trainable(inputs, temperature=config.temperature)  
                predicted = outputs.argmax(dim=-1)
        ...
        print(f"Test Accuracy: {correct / total:.4f}")
        acc_trainable_rasp.append(correct / total)

    # Run training for both CNN models
    print("Training Classification CNN:")
    train_cnn_model(model_cnn, is_regression=False, loss_list=loss_cnn_model)
    
    print("\nTraining Regression CNN:")
    train_cnn_model(model_cnn_reg, is_regression=True, loss_list=loss_cnn_reg_model)

    print("\nTraining Transformer Classifier:")
    train_cnn_model(model_transformer_cls, is_regression=False, loss_list=loss_transformer_cls_model)

    print("\nTraining Transformer Regressor:")
    train_cnn_model(model_transformer_reg, is_regression=True, loss_list=loss_transformer_reg_model)

    model_trainable = Model_1D_trainable(model_config=config.model_config,
                          model_parameters=config.model_parameters,
                          unembedding_data=config.unembed_matrix,
                          encoding_func=config.encoding_func,
                          encoded_vocab=config.encoded_vocab,
    ...
    train_model_train(model_trainable=model_trainable)
    evaluate_model_train(model_trainable=model_trainable)
...
def evaluate_per_count(models_dict, m=100):
    ...
    original_model, cnn_model, cnn_reg_model, transformer_cls, transformer_reg = models_dict['original'], models_dict['cnn'], models_dict['cnn_reg'], models_dict['transformer_cls'], models_dict['transformer_reg']
    max_count = 25  # From dataset setup
    results = {model_name: np.zeros(max_count+1) for model_name in models_dict.keys()}
    
    # Enhanced EvalDataset with persistence
    class EvalDataset(Dataset):
        def __init__(self, load_from_file=False, filename='eval_dataset.pth'):
            ...
            self.required_params = {
                ...
                'tmp': config.tmp,
                'sep': config.sep,
                'token_to_count': config.token_to_count,
            ...
            if load_from_file and os.path.exists(filename):
                ...
                if self._validate(loaded):
                    ...
                    print(f"Loaded evaluation dataset from {filename}")
                    return
                
            # Generate new data if no file or validation failed
            self.data = []
            for count in range(max_count+1):
                for _ in range(m):
                    ...
                    np.random.shuffle(seq[3:])
                    seq = seq[:config.max_seq_len]
                    self.data.append(seq)
            self._save()
            print(f"Created and saved new evaluation dataset to {filename}")

        def _validate(self, loaded_data):
            for k, v in self.required_params.items():
                if loaded_data.get(k) != v:
                    print(f"Param mismatch: {k} (saved {loaded_data.get(k)} vs current {v})")
                    return False
            return True
        ...
        def _save(self):
            ...
            torch.save(save_data, self.filename)
    ...
    # Create dataset with persistence
    eval_loader = DataLoader(
        EvalDataset(load_from_file=True), 
        batch_size=64, 
        shuffle=False, 
    ...
    with torch.no_grad():
        for batch in eval_loader:
            inputs, _ = batch
            # Original model
            original_outputs = original_model(inputs, temperature=0.0000000001, hard=True).argmax(-1)
            original_counts = original_outputs[:, 1].cpu().numpy()
            
            # CNN Classifier
            encoded = model.encoder(inputs)
            cnn_class_counts = cnn_model(encoded).argmax(-1).cpu().numpy()
            
            # CNN Regressor
            cnn_reg_counts = cnn_reg_model(encoded).round().long().cpu().numpy()

            transformer_cls_counts = models_dict['transformer_cls'](encoded).argmax(-1).cpu().numpy()
            transformer_reg_counts = models_dict['transformer_reg'](encoded).round().long().cpu().numpy()

            model_trainable_outputs = model_trainable(inputs, temperature=0.1, hard=True).argmax(-1)
            model_trainable_counts = model_trainable_outputs[:, 1].cpu().numpy()

            
            true_counts = [seq[3:].count(config.token_to_count) for seq in inputs]
            
            # Update all results
            for tc, oc, cc, cr, tcl, tr, mtc in zip(true_counts, original_counts, cnn_class_counts, cnn_reg_counts, transformer_cls_counts, transformer_reg_counts, model_trainable_counts):
                results['original'][tc] += (oc == tc + 1)
                results['cnn'][tc] += (cc == tc)
                results['cnn_reg'][tc] += (cr == tc)
                results['transformer_cls'][tc] += (tcl == tc)
                results['transformer_reg'][tc] += (tr == tc)
                results['model_trainable'][tc] += (mtc == tc + 1)

        # Convert to percentages
        for key in results:
            results[key] = (results[key] / m) * 100
            
        return results

def plot_count_accuracy(results):
    counts = np.arange(26)
    width = 0.10  # Adjusted for 5 models
    
    fig, ax = plt.subplots(figsize=(20, 8))
    
    rects1 = ax.bar(counts - 2*width, results['original'], width, label='Original')
    rects2 = ax.bar(counts - width, results['cnn'], width, label='CNN Cls')
    rects3 = ax.bar(counts, results['cnn_reg'], width, label='CNN Reg')
    rects4 = ax.bar(counts + width, results['transformer_cls'], width, label='Transformer Cls')
    rects5 = ax.bar(counts + 2*width, results['transformer_reg'], width, label='Transformer Reg')
    rects6 = ax.bar(counts + 3*width, results['model_trainable'], width, label='Trainable RASP')
...
models_to_evaluate = {
    'original': model,
    'cnn': model_cnn,
    'cnn_reg': model_cnn_reg,
    'transformer_cls': model_transformer_cls,
    'transformer_reg': model_transformer_reg,
...
data = {
    ...
    'loss_cnn_reg_model': loss_cnn_reg_model,
    'loss_transformer_cls_model': loss_transformer_cls_model,
    'loss_transformer_reg_model': loss_transformer_reg_model,
    'loss_trainable_rasp' : loss_trainable_rasp,
    'acc_rasp_model': acc_rasp_model,
    'acc_cnn_model': acc_cnn_model,
    'acc_cnn_reg_model': acc_cnn_reg_model,
    'acc_transformer_cls_model': acc_transformer_cls_model,
    'acc_transformer_reg_model': acc_transformer_reg_model,
    'acc_trainable_rasp' : acc_trainable_rasp,
}

with open('run1_data750.json', 'w') as f:
